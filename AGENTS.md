...
### `presentation/` — slides
- `slides.pptx` — **11 widescreen slides** built by `build_pptx.py`. Speaker
  notes pre-filled per teammate. Real numbers in the Results table. The
  five generated narrative figures are embedded directly.
- `slides_outline.md` — per-slide bullet skeleton + timing budgets summing
  to ≤ 5:00 (the hard rubric cap).
- `NARRATIVE.md` — **the full story you tell, slide-by-slide**, with
  hooks, transitions, and what to point at on each visual. Read this
  to rehearse.
- `build_pptx.py` — re-run `python build_pptx.py` if you change content;
  expects the 5 PNGs in `report/figures/` (see below).

### `report/figures/` — narrative figures used by both report and slides
- `pipeline.png`, `qualitative.png`, `failures.png` — the three figures
  required by the LaTeX report (`extract_figures.py`).
- `pipeline_visual.png` — 4-panel pipeline diagram (raw → YOLO → ROIs →
  IoU output) used as the hero of Slide 4.
- `roi_labeling.png` — mockup screenshot of `roi_picker.py` mid-click,
  used on Slide 5 (left).
- `gt_labeling.png` — mockup screenshot of `label_gt.py` with one spot
  highlighted yellow, others red/green/grey, used on Slide 5 (right).
- `results_chart.png` — bar chart, synthetic vs UTD live, hero of Slide 6.
- `precision_recall.png` — recall iteration chart (64% → 80% → 98%)
  used on Slide 8.
- All five generated by `code/make_presentation_visuals.py`.
...
- 2026-04-29: Built the presentation narrative pass. Wrote `code/make_presentation_visuals.py` (single script that generates 5 narrative PNGs into `report/figures/`: `roi_labeling.png` mockup of the ROI picker, `gt_labeling.png` mockup of the GT labeler, `pipeline_visual.png` 4-panel raw→YOLO→ROIs→output diagram, `results_chart.png` synthetic-vs-UTD bar chart, `precision_recall.png` 64%→80%→98% recall iteration chart). Wrote `presentation/NARRATIVE.md` — full slide-by-slide speaker script with the requested story arc (parking circle hook → existing-solutions failure modes → our methodology → 97.6% accuracy → daily-life impact). Restructured `presentation/build_pptx.py` to embed all 5 figures and added a new "What's wrong with existing solutions" comparison table slide; rebuilt `slides.pptx` (now 11 slides). Updated `slides_outline.md` to point at `NARRATIVE.md` and updated this file's section 2 to document the new figure set.
