...
We recorded a clip at \emph{848$\times$464} / 30~fps from an
upper-floor walkway overlooking an adjacent on-campus surface lot, at
a downward angle of approximately~$30^{\circ}$. The recording is
41.1\,s long (1233 frames) and the camera was kept fully static
(start and end frames are visually indistinguishable). The visible
portion of the lot contains three rows: a fully-unoccluded front row
of large spots and two partially-occluded back rows. We registered 17
parking-spot ROIs covering all three rows with \texttt{roi\_picker.py}.
For evaluation we annotated 5 uniformly-spaced frames with per-spot
occupancy labels using \texttt{label\_gt.py}, yielding $5 \times 17 =
85$ binary judgments. At runtime the system displays both an
\emph{Occupied} count (red) and an \emph{Open} count (green) on the
live HUD; on this clip the predicted occupancy cycles between 6 and 8
spots (mean 7.3) over the 41.1\,s window.
...
\midrule
Inference FPS (YOLOv8n, $640{\times}640$) & 5.9      & 17.3 \\
Inference FPS (YOLOv8n, $960{\times}960$) & 3.0      & --   \\
End-to-end FPS (incl. I/O and rendering)  & 4.8      & 15.7 \\
\bottomrule
\end{tabular}
\caption{Quantitative results. The \emph{Synthetic} column reports
         numbers from a 90-frame synthetic stress test (810 (frame, spot)
         judgments, exact ground truth) using YOLOv8n on a laptop CPU.
         The \emph{UTD live} column reports numbers from a 41\,s,
         1233-frame, 17-spot recording of an on-campus surface lot at a
         $\sim$30$^\circ$ elevation, evaluated against 85 manually
         labelled (frame, spot) judgments across 5 evenly-spaced frames.
         The detector reaches 97.6\,\% occupancy accuracy, with only two
         per-judgment errors out of 85: one false negative and one false
         positive on adjacent partially-occluded back-row spots in a
         single frame, where a vehicle straddles the boundary between
         two narrow ROIs.}
\label{tab:results}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Extreme top-down views.} On a publicly available
          ...
          for UTD deployment.
    \item \textbf{Partial-coverage ROIs in occluded back rows.} The
          most common source of error during our UTD development was
          insufficient ROI coverage of the visible vehicle. For back-row
          spots whose painted boundaries are partially occluded by
          front-row vehicles, an ROI that encompasses only the visible
          spot floor is smaller than YOLO's bounding box around the
          entire visible vehicle. The resulting IoU lands at
          $\approx$0.4--0.49, just below the 0.5 threshold, and the
          spot is reported \emph{Empty}. We quantified this effect via
          three iterations of manual ROI registration on the same 17
          UTD spots: tight ``visible-floor'' ROIs gave 63.8\,\% recall;
          ROIs sized to the full image extent of each parked car lifted
          recall to 79.7\,\%; and a third iteration that explicitly
          enlarged the three foreground ROIs whose vehicles dominate
          the frame lifted recall further to 98.2\,\%. The remaining
          two errors (one false positive, one false negative) occur in
          a single frame on adjacent narrow back-row spots where a
          vehicle straddles the ROI boundary -- a true ambiguity that
          neither auto- nor manual ROI registration can fully resolve.
\end{itemize}
